...
```bash
make fuzz-test         # run 7 fuzz targets for 10s each (requires fuzz preset)
make fuzz-long         # run all targets for 5min each (soak before releases)
```
...
Per-target `-max_len` and `-timeout` are configured in the Makefile.
