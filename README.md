...
```bash
just build PRESET=fast   # GCC + Ninja + lld + unity (one-time full builds)
just build              # GCC + Ninja, per-file (editor + incremental dev loop)
...
```bash
just test-unit         # fast in-process tests
just test-sim          # deterministic simulation tests (SimClock/SimBus)
just test-integration  # forks real cinderd processes (RUN_SERIAL)
just test-cli          # forks cinderd + cinder-cli
just test-all          # run every test binary against the current preset
just test              # everything via ctest
```

### Fuzz testing

```bash
just fuzz-test         # run 7 fuzz targets for 10s each (requires fuzz preset)
just fuzz-long         # run all targets for 5min each (soak before releases)
```
...
│   │   ├── snapshot_fuzz.cpp
│   │   ├── wal_fuzz.cpp
│   │   ├── anti_entropy_fuzz.cpp
│   │   ├── http_parse_fuzz.cpp
│   │   ├── generate_corpus.py
│   │   └── corpus/              # 90+ seed files across 7 directories
